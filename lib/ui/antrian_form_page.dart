...
class _AntrianFormPageState extends State<AntrianFormPage> {
  ...
  void _searchPasien() {
    ...
    setState(() {
      _filteredPasien = _allPasien.where((p) {
        return boyerMooreContains(p.namaPasien ?? '', query) || 
               boyerMooreContains(p.nomorRMPasien ?? '', query);
      }).toList();
    });
  }
    ...
    return StreamBuilder<List<Poli>>(
      ...
      builder: (context, snapshot) {
        ...
        return DropdownButtonFormField<Poli>(
          ...
          value: _selectedPoli,
          items: list.map((e) => DropdownMenuItem(value: e, child: Text(e.nm_poli ?? ''))).toList(),
          onChanged: (val) {
  ...
  Widget _buildJadwalDropdown() {
    ...
    List<DropdownMenuItem<JadwalPoli?>> items = [
      DropdownMenuItem(value: null, child: Text("Walk-in / Tanpa Sesi Terjadwal"))
    ];
    ...
    for (var j in _jadwalList) {
      ...
      items.add(DropdownMenuItem(value: j, child: Text("Sesi: ${j.jamMulai} - ${j.jamSelesai}")));
    }
      ...
      children: [
        if (_jadwalList.isEmpty)
          Padding(
            ...
            child: Text("Tidak ada jadwal dokter untuk poli ini hari ini", style: TextStyle(color: Colors.red)),
          ),
  ...
  Widget _buildPasienSearch() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Text("3. Pilih Pasien", style: TextStyle(fontWeight: FontWeight.bold)),
            Row(
              children: [
                Expanded(
                  child: TextField(
  ...
  Future<void> _daftarAntrian() async {
    if (_selectedPoli == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Pilih poli terlebih dahulu")));
      return;
    }
    if (_selectedPasien == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Pilih pasien terlebih dahulu")));
      return;
    }
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text("Pendaftaran Berhasil", textAlign: TextAlign.center),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("Nomor Antrian:"),
              Text(nomor, style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Colors.blue)),
            ],
          ),
          actions: [
            ElevatedButton(
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => LayarAntrianPage()));
              }, 
